...
dataset_name = 'archi'
dataset_dir = f"archi-ordered-effect/{dataset_name}"
columns = ['node_cls_label', 'edge_removal', 'type_semantic_removal', 'distance', 'cleanse', 'ordered', 'use_node_types', 'use_edge_types']
metrics = ['eval_accuracy', 'eval_macro_precision', 'eval_macro_recall', 'eval_macro_f1']
header = ['Config', 'Target', 'Edge Fraction', 'Percentage Masked', 'Hops', 'Cleansed', 'Ordered', 'Use Node Types', 'Use Edge Types'] + ["Accuracy", "Precision", "Recall", "F1"]
...
for config in os.listdir(dataset_dir):
    ...
    if best_result and run_config:
        row = [run_config[k] for k in columns] + [best_result[k] for k in metrics]
...
for target_name in df['Target'].unique():
    target_df = df.loc[df["Target"] == target_name]
    config_columns = ['Edge Fraction', 'Percentage Masked', 'Hops', 'Cleansed', 'Ordered', 'Use Node Types', 'Use Edge Types']
    column_results = dict()
...
with open(f'{dataset_name}-comparative.json', 'w') as f:
