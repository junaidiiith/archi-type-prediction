...
from ..utils.file import find_files_with_extension
...
class OntoUMLNxG(LangGraph):
        ...
        def create_nxg():
            ...
            for k, v in id2obj_map.items():
                ...
                if v[ONTOUML_ELEMENT_TYPE] in [ONTOUML_CLASS, ONTOUML_RELATION]:
                    self.add_node(k, name=node_name,
                                  type=v[ONTOUML_ELEMENT_TYPE], description='')
                    for prop in extra_properties:
                        self.nodes[k][prop] = v[prop] if prop in v else False

                    logger.info(
                        f"Node: {node_name} type: {v[ONTOUML_ELEMENT_TYPE]}")
                # else:
                #     continue
                ...
                if ONTOUML_ELEMENT_DESCRIPTION in v and v[ONTOUML_ELEMENT_DESCRIPTION] is not None:
                    ...
                    logger.info(
                        f"Description: {v[ONTOUML_ELEMENT_DESCRIPTION]}")

                if v[ONTOUML_ELEMENT_TYPE] == ONTOUML_CLASS:
                    if ONTOUML_CLASS_LITERALS in v and v[ONTOUML_CLASS_LITERALS] is not None:
                        literals = v[ONTOUML_CLASS_LITERALS] if isinstance(
                            v[ONTOUML_CLASS_LITERALS], list) else [v[ONTOUML_CLASS_LITERALS]]
                        literals_str = ", ".join(
                            [literal[ONTOUML_ELEMENT_NAME] for literal in literals])
                        ...
                        properties = v[ONTOUML_PROPERTIES] if isinstance(
                            v[ONTOUML_PROPERTIES], list) else [v[ONTOUML_PROPERTIES]]
                        self.nodes[k][ONTOUML_PROPERTIES] = [
                            property[ONTOUML_ELEMENT_NAME] for property in properties]
                    ...
                    logger.info(
                        f"\tGeneralization:, {specific_name} -->> {general_name}\n")
    ...
    def copy(self) -> "OntoUMLNxG":
        """Return a deep copy of the OntoUML graph."""
        return super().copy()
...
def get_ontouml_to_nx(data_dir, min_stereotypes=10):
    ...
    for mfp in tqdm(models, desc=f"Reading {len(models)} OntoUML models"):
        if mfp.endswith(".ecore") or mfp.endswith(".json"):
            ...
            stereotype_nodes = [node for node, stereotype in g.nodes(
                data=ONTOUML_STEREOTYPE) if stereotype is not None]
